...
if ($_POST['step'] == '1'){
	...
	if (empty($install_hote) or empty($install_user) or empty($install_base)){
		...
		if(!($connexion = mysqli_connect($install_hote, $install_user, $install_pass, $install_base))){
			$_SESSION['report'] = 'Connexion à la Base de donnée impossible';
			header('Location: step1.php');
			exit();
		}
		if(!($db = mysqli_select_db($connexion, $install_base))){
...
elseif ($_POST['step'] == '2')
{
  ...
  else
  {
    ...
    $connexion = mysqli_connect($_SESSION['install_hote'], $_SESSION['install_user'], $_SESSION['install_pass'], $_SESSION['install_base']);
    $db = mysqli_select_db($connexion, $_SESSION['install_base']);

    if ($_SESSION['install_notable'] == '0')
    {
      mysqli_query($connexion, "DROP TABLE IF EXISTS {$_SESSION['install_tablemembre']}");
      mysqli_query($connexion, "DROP TABLE IF EXISTS {$_SESSION['install_tableconf']}");
    }
